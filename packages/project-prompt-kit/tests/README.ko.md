...
v0.1.x의 테스트 기반 구조(test infrastructure)는 의도적으로 최소화되어 있습니다.
...
- `fixtures/invalid/`는 필수 필드(required field), 열거형(enum), const 값(const value), 타입 검사(type check), 문자열/배열 최솟값(string/array minimum), 추가 속성(extra property), 안전하지 않은 안전 기본값(unsafe safety default), 중첩된 안전 구조(nested safety shape), 지원하지 않는 스키마 키워드(unsupported schema keyword)에 대한 독립 실패 사례를 담습니다.
